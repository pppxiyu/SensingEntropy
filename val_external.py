...
sensors_select = sensors[: round(len(sensors) * 0.33)]  # use top 33% sensors
...
for i in road_data.flood_time_citywide.copy().index:
    ...
    inundation_observed = [k for k in inundation if k in sensors_select]
    if inundation_observed:
        ...
        marginals_flood = bayes_network_t.marginals_downward.copy()
